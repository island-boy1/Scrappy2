...
async function openDetail(row){$('detailModal').hidden=false;$('detailContent').innerHTML=`<div class="detail-head"><div><p class="eyebrow">VEHICLE DETAIL</p><h2>${esc(row.year)} ${esc(row.make)} ${esc(row.model)}</h2><p>${esc(row.submodel||'')} · ${esc(row.yard)}</p></div>${sourceBadge(row)}</div><div class="detail-grid"><div><strong>VIN</strong><span>${esc(row.vin||'Not published')}</span></div><div><strong>Stock</strong><span>${esc(row.stock||'Not published')}</span></div><div><strong>Row</strong><span>${esc(row.row||'Not published')}</span></div><div><strong>Arrival</strong><span>${esc(row.arrival||'Not published')}</span></div><div><strong>Body</strong><span>${esc(row.body||'Not published')}</span></div><div><strong>Drive</strong><span>${esc(row.drive||'Not published')}</span></div></div><div id="photoArea" class="photo-area">Checking for a published vehicle photo…</div><div class="source-detail"><strong>Inventory provenance</strong><span>${esc(row.sourceLabel||row.sourceType||'Unknown')} · ${esc(row.sourceTrust||'MANUAL')}</span>${row.sourceTrust==='FALLBACK'?'<small>Fallback records should be verified with the yard before travel.</small>':''}</div><div class="detail-actions">${row.sourceUrl?`<a class="primary-link" href="${esc(row.sourceUrl)}" target="_blank" rel="noopener">Open yard vehicle/source page</a>`:''}${row.vin?`<button class="secondary" onclick="decodeDetailVIN('${esc(row.vin)}')">Decode VIN</button>`:''}</div><div id="detailDecode" class="vin-result"></div></div>`; if(row.sourceUrl){try{const r=await fetch('/api/vehicle?url='+encodeURIComponent(row.sourceUrl));const d=await r.json();if(d.image){$('photoArea').innerHTML=`<img src="${esc(d.image)}" alt="${esc(row.year+' '+row.make+' '+row.model)}" class="vehicle-photo"><p class="muted">Photo published by the yard/source.</p>`}else $('photoArea').innerHTML='<p class="muted">No public vehicle photo was found on this source page.</p>'}catch(e){$('photoArea').innerHTML='<p class="muted">Photo lookup unavailable; use the source link above.</p>'}}}
